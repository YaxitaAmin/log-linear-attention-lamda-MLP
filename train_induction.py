...
def generate_induction(n_samples, seq_len, vocab_size=64, pattern_len=1, seed=42):
    """
    Induction heads task:
    - Sequence looks like: [prefix tokens] [trigger] [pattern] ... [trigger] [???]
    - Model must predict the token that followed [trigger] the first time it saw it.

    Structure per sample:
        [random prefix of length prefix_len]
        [trigger token A]
        [pattern token B]   <- model must recall B when it sees A again
        [random filler tokens]
        [trigger token A]   <- repeated trigger
        label = B           <- model predicts B here

    So the gap between first A and second A is long — tests long-range induction.
    """
    np.random.seed(seed)
    assert seq_len >= pattern_len * 2 + 4, "seq_len too short for induction task"

    input_ids = np.zeros((n_samples, seq_len), dtype=np.int64)
    labels    = np.full((n_samples, seq_len), -100, dtype=np.int64)

    # token 0 reserved for padding, use 1..vocab_size-1
    for b in range(n_samples):
        # pick a trigger token and a pattern token (must be different)
        trigger = np.random.randint(1, vocab_size // 2)
        pattern = np.random.randint(vocab_size // 2, vocab_size)

        # fill entire sequence with random noise first
        seq = np.random.randint(1, vocab_size, size=seq_len)

        # place [trigger, pattern] near the start (position 1 and 2)
        seq[1] = trigger
        seq[2] = pattern

        # place repeated trigger near the end
        trigger_pos = seq_len - 2
        seq[trigger_pos] = trigger

        # the label at trigger_pos+1 is pattern
        labels[b, trigger_pos + 1] = pattern
...
def train(lambda_mode, model_dim, seq_len, vocab_size, batch_size,
          max_steps, lr, mlp_hidden_dim, seed, device,
          output_dir="results/induction"):

    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)

    run_name = f"{lambda_mode}_seq{seq_len}_seed{seed}"
    ...
    train_x, train_y = generate_induction(n_train, seq_len, vocab_size, seed=seed)
    val_x,   val_y   = generate_induction(n_val,   seq_len, vocab_size, seed=seed+1)
    ...
    print(f"\n{'='*60}")
    print(f"Mode: {lambda_mode} | dim: {model_dim} | seq: {seq_len} | seed: {seed}")
    print(f"lr: {lr} | vocab: {vocab_size}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir",     type=str,   default="results/induction")
