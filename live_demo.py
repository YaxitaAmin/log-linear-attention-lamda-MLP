...
def run_demo(num_kv_pairs_choice, seq_len_choice, seed_choice):
    ...
    # pick the right checkpoint for this kv+seq combo
    key_sp = f"mlp_softplus_kv{kv}_seq{seq_len}"
    key_fx = f"fixed_kv{kv}_seq{seq_len}"
    model_sp = MODELS.get(key_sp)
    model_fx = MODELS.get(key_fx)

    if model_sp is None or model_fx is None:
        msg = f"No checkpoint loaded for kv={kv} seq={seq_len}!\nAvailable: {list(MODELS.keys())}"
        return msg, None
...
def build_ui():
    with gr.Blocks(title="Adaptive Memory Decay — Live Demo") as demo:
...
# ── all checkpoints to preload ────────────────────────────────────────────────
# (kv, seq) -> available for both softplus and fixed
ALL_CONFIGS = [
    (4,  256),
    (8,  256),
    (16, 256),
...
def main():
    ...
    parser.add_argument("--results_dir", type=str,
                        default="results/mqar",
                        help="Root results dir containing all run folders")
    parser.add_argument("--seed",        type=int, default=0,
                        help="Which seed checkpoint to load (default 0)")
    ...
    print(f"\nDevice : {device}")
    print(f"Loading all checkpoints from {ARGS.results_dir}...\n")

    loaded = 0
    for (kv, seq) in ALL_CONFIGS:
        num_levels = get_num_levels(seq, base=LEVEL_BASE)
        for mode in ["mlp_softplus", "mlp_softmax", "fixed"]:
            seed = ARGS.seed
            if seq == 512 and seed == 2:
                continue
            run_name = f"{mode}_kv{kv}_seq{seq}_seed{seed}"
            ckpt     = os.path.join(ARGS.results_dir, run_name, "best_model.pt")
            if not os.path.exists(ckpt):
                print(f"  [skip] not found: {run_name}")
                continue
            key = f"{mode}_kv{kv}_seq{seq}"
            MODELS[key] = load_model(
                ckpt, mode, num_levels,
                ARGS.model_dim, ARGS.mlp_hidden
            ).to(device)
            loaded += 1

    print(f"\nLoaded {loaded} checkpoints! Starting Gradio demo...\n")
    demo = build_ui()
