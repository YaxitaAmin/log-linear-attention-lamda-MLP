...
class HAttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, state_size, num_levels,
        ...
        if lambda_mode == "mlp_softplus":
            self.lambda_module = LambdaMLPSoftplus(
                num_levels=num_levels, hidden_dim=mlp_hidden_dim)
    ...
    def forward(self, x):
        ...
        # decay: g = -exp(A_log) * softplus(dt + dt_bias)  [always negative]
        A  = -torch.exp(self.A_log.float())
        dt = self.dt_proj(x)               # (b t h)
        dt = F.softplus(dt + self.dt_bias) # (b t h) positive
        g  = A[None, None, :] * dt         # (b t h) negative

        # lambda — exactly matches compute_lambda in mamba_apis.py
        if self.lambda_fixed:
            # baseline: softplus(L * dl)
            L = F.softplus(self.L[None, None, :, :] * dl)
        elif self.lambda_mode == "mlp_softplus":
...
class MQARModel(nn.Module):
    def __init__(self, vocab_size, d_model, num_heads, state_size,
        ...
        # no weight tying - causes scale issues with large vocab
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                nn.init.normal_(m.weight, std=0.02)
        # init out_proj smaller to prevent explosion
...
def train(lambda_mode, model_dim, num_kv_pairs, seq_len, batch_size,
    ...
    vocab_size  = 128  # smaller vocab for tiny model
